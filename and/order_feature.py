...
order = pd.read_csv('../newdata/t_order.csv')
...
for d in date_list:
    ...
    all_feature['dt'] = d
    #优惠金额和订单数的比值
    #all_feature['offer_ord_rate']=all_feature.offer_amt_sum.astype('float')/all_feature.ord_cnt_sum

    #销售总金额和下单顾客数的比值
    all_feature['sale_user_rate']=all_feature.sale_amt_sum.astype('float')/all_feature.user_cnt_sum
    #销售总金额和订单数的比值
    all_feature['sale_ord_rate']=all_feature.sale_amt_sum.astype('float')/all_feature.ord_cnt_sum
    #销售总金额和优惠总金额的比值
    all_feature['sale_offer_rate']=all_feature.offer_amt_sum.astype('float')/all_feature.sale_amt_sum
    #销售金额最大值和顾客数最大值
    all_feature['sale_uer_max']=all_feature.sale_amt_max.astype('float')/all_feature.user_cnt_max
    #销售金额的均值和订单量的均值
    all_feature['sale_ord_mean']=all_feature.sale_amt_mean.astype('float')/all_feature.ord_cnt_mean
    #销售金额的均值和订单量的总数
    all_feature['sale_mean_ord']=all_feature.sale_amt_mean.astype('float')/all_feature.ord_cnt_sum
    #订单总量和下单顾客数总量
    all_feature['ord_user']=all_feature.user_cnt_sum.astype('float')/all_feature.ord_cnt_sum


    feature = pd.concat([feature, all_feature])
    print(d)
feature.to_csv('../newdata/feature_all.csv', index=False)
print('finish')