...
if __name__=='__main__':
 ...
 #获得订单日期在 2016-8 到 2017-3之间的数据
    t_order_data_limt1=t_order_data[t_order_data['ord_dt'].map(lambda x:True if '2017-04-01' <=str(x)<='2017-04-30'else False)]
...
#获得店铺评论在2016-8到2017-3之间
    t_comment_data_limt1=t_comment_data[t_comment_data['create_dt'].map(lambda x:True if '2017-04-01' <=str(x)<='2017-04-30'else False)]
...
# #在2016-8 到2017-3中店铺的广告信息
    t_ads_data_limt1=t_ads_data[t_ads_data['create_dt'].map(lambda x:True if '2017-04-01' <=str(x)<='2017-04-30'else False)]
...
#     feature01=pd.merge(feature01,label02,on='shop_id',how='left')
    feature01.to_csv('../data/pa/test1111.csv')
