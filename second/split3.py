...
#获得店铺评论在2016-8到2017-3之间
    t_comment_data_limt1=t_comment_data[t_comment_data['create_dt'].map(lambda x:True if '2016-10-01' <=str(x)<='2016-10-31'else False)]
...
# #在2016-8 到2017-3中店铺的广告信息
    t_ads_data_limt1=t_ads_data[t_ads_data['create_dt'].map(lambda x:True if '2016-10-01' <=str(x)<='2016-10-31'else False)]
...
# #在2016-8 到2017-3商品
    t_product_data_limt1=t_product_data[t_product_data['on_dt'].map(lambda x:True if str(x)<='2016-10-31'else False)]
...
# #后一个月的销售额
#     t_order_data_limt2=t_order_data[t_order_data['ord_dt'].map(lambda x:True if '2016-11-01' <=str(x)<='2016-11-30'else False)]
...
# # #2017-4的销售额
    ...
    # t_sales_sum_data_limt2=t_sales_sum_data[t_sales_sum_data['dt'].map(lambda x:True if str(x)=='2016-08-31'else False)]
    ...
    feature01.to_csv('../data/pa/feature03333.csv')